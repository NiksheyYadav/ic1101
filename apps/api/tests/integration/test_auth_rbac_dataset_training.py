...
def token(role: str = "admin") -> str:
    response = client.post("/v1/auth/token", json={"user_id": "u-1", "role": role})
...
def test_dataset_upload_and_versioning_happy_path() -> None:
    ...
    r1 = client.post("/v1/datasets", data={"name": "train-ds"}, files=files, headers={"Authorization": f"Bearer {t}"})
    assert r1.status_code == 201
    assert len(r1.json()["versions"]) == 1

    files2 = {"file": ("sample2.csv", io.BytesIO(b"a,b\n5,6\n"), "text/csv")}
    r2 = client.post("/v1/datasets", data={"name": "train-ds"}, files=files2, headers={"Authorization": f"Bearer {t}"})
...
def test_training_job_lifecycle_and_events() -> None:
    t = token("admin")
    created = client.post("/v1/training-jobs", headers={"Authorization": f"Bearer {t}"})
...
def test_rbac_viewer_cannot_create_training_job() -> None:
    t = token("viewer")
    response = client.post("/v1/training-jobs", headers={"Authorization": f"Bearer {t}"})
