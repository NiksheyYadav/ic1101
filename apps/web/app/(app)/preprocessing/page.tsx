...
export default function PreprocessingPage() {
  ...
  const steps = activePipeline && activePipeline.steps && activePipeline.steps.length > 0 
                ? activePipeline.steps.map((s: any) => ({...s, color: s.color || "var(--cyan)", icon: s.icon || "⚙️"})) 
                : defaultSteps;

  return (
    <div className="animate-in">
      <div className="page-header">
        <div><h2>Preprocessing Pipeline</h2><div className="breadcrumb">{activePipeline?.name || "Customer Churn Pipeline"}</div></div>
